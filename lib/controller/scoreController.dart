...
class ScoreController extends GetxController {
  ...
  Future<void> scorePost(int post_id) async {
    try {
      ...
      var url =
          Uri.parse(ApiEndPoints.baseUrl + ApiEndPoints.authEndpoints.scoreAdd);
      Map body = {
        'score_num': scoreController.text.trim(),
        'user_id': user_id,
        'post_id': post_id,
      };
  ...
  Future<void> updateScore(int post_id) async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      String? token = prefs.getString("token");
      print(token);
      ...
      var url = Uri.parse(ApiEndPoints.baseUrl +
          ApiEndPoints
              .authEndpoints.scoreAdd); // ปรับเปลี่ยน URL ตามความเหมาะสม
